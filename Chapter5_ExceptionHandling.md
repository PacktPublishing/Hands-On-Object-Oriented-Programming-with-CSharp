...
The software will let us know by passing a message (which we call exception in programming). But what should it say to us. "Help! Help!" won't make our life easy. We need more information about the situation, so that we can guide the computer to work accordingly. So the .Net framework has created some very common exceptions which occurs very often. If the problem that the software is facing has an predefined exception, it will throw that. For example, if there is a case where the program is trying to divide a number zy zero but mathematically its not possible, so the computer on the application is in big trouble. Its confused and helpless. It tries to think any way out but it can't find any solution and decides to die because he always performs what the software developers say it to do, it doesn't have any intelligence. But in .NET framework there is such kind of exception already defined which is DivideByZeroException. If this type of situation occurs then don't get panicked and don't try to die, look for a catch block which handles this situation. The catch block should tell you a way out, it should handle the situation. So that is why we need exceptions in programming. And that is why its super important that you know about exception handling.
